...
int randrange(int num_tickets) {
  return 1 + (do_rand(&rand_next) % num_tickets);
}
...
found:
  ...
  p->ctime = ticks;
  p->count = 0;
  p->mask = (p->parent) ? p->parent->mask : 0;
  
  #ifdef SCHEDULER_LBS
    p->num_tickets = (p->parent) ? p->parent->num_tickets : 1;
...
void exit(int status)
{
  ...
  // get the parent proc struct and update the value
  struct proc *parent_proc = p->parent;
  parent_proc->count += p->count;
...
void lbs_sched() {
  ...
  for (;;) {
    ...
    // generate a random number between 1 and num_tickets
    lottery = randrange(total_tickets);
    
    // find the process which won the lottery
    int tickets_so_far = 0;
    for (int i = 0; i < NPROC; i++) {
      p = &proc[i];
      if (p->state == RUNNABLE) {
        tickets_so_far += p->num_tickets;
        if (tickets_so_far >= lottery) {
          winner = p;
          break;
        }
      }
    ...
    // winner has won the lottery.
    // now check for any process with same number of tickets with early arrival time
    if (winner) {
      for (int j = 0; j < NPROC; j++) {
        p = &proc[j];
        if (p->state == RUNNABLE && p->num_tickets == winner->num_tickets && p->arrival_time < winner->arrival_time) {
          winner = p;
          break;
        }
      }
      ...
      winner = proc; // if no winner found, then schedule the first process
    }
...
#ifdef SCHEDULER_MLFQ
int higher_priority(int priority) {
